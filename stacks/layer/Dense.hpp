...
namespace sx
{
	...
	class Dense :
		...
		LDWeights<T, SZ_IN * SZ_OUT, needBufM<T,FN_OPTIM>(), needBufV<T,FN_OPTIM>()>,
		LDBiases<T, SZ_OUT, needBufM<T,FN_OPTIM>(), needBufV<T,FN_OPTIM>()>
	// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	{
		// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		SX_FNSIG_LAYER_EXE final
		{
			for(auto o = uMAX(0); o < SZ_OUT; ++o)
			{
				...
				{
					this->OutRaw[o] = std::inner_product(this->Input, this->Input + SZ_IN, this->Weights + math::index_c(0, o, SZ_IN), T(0));
